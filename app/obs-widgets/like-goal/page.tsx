...
function LikeGoalContent() {
  ...
  if (authStatus === 'checking') return (
      <div className="inline-block m-0 p-0 leading-none"><div className="bg-black/90 border border-white/10 p-6 rounded-2xl flex flex-col items-center gap-4 shadow-2xl"><Loader2 className="animate-spin text-blue-500 w-8 h-8" /><p className="text-white font-black text-xs tracking-widest uppercase">Verifying License Key...</p></div></div>
  );

  if (authStatus === 'invalid') return (
      <div className="block m-0 p-0 w-fit h-fit overflow-hidden bg-transparent"><div className="bg-red-500/10 border border-red-500/50 p-6 rounded-2xl flex flex-col items-center gap-4 shadow-2xl backdrop-blur-md"><ShieldAlert className="text-red-500 w-12 h-12" /><p className="text-red-500 font-black text-sm tracking-widest uppercase">Access Denied: Invalid Key</p></div></div>
  );
