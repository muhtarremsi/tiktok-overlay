    ...
    <div className="flex h-screen w-full overflow-hidden bg-[#09090b] text-zinc-200 font-sans text-[12px]">
      <aside className="w-64 bg-black border-r border-white/10 flex flex-col p-5">
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="bg-[#0c0c0e] border border-zinc-800/50 rounded-xl p-4 space-y-3 font-bold uppercase tracking-widest text-[9px] text-zinc-500">
            ...
            <div className="flex justify-between items-center"><span><Key size={14} className="inline mr-2 text-blue-500"/> License</span><span className="text-blue-500 font-black">PRO</span></div>
      ...
      <main className="flex-1 bg-[#09090b] flex flex-col">
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 font-bold uppercase tracking-widest text-[10px]">
          ...
          {isLive && <span className="text-green-500 flex items-center gap-2 tracking-[2px]"><div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_#22c55e]" /> LIVE</span>}
        </header>
        <div className="flex-1 overflow-y-auto p-10">
          {activeView === "ttv" ? <ModuleTTV username={username} baseUrl={baseUrl} isLive={isLive} /> : 
           activeView === "settings" ? <div className="p-10 text-zinc-600 font-black italic uppercase">Settings Module Active</div> :
...
function ModuleTTV({ username, baseUrl, isLive }: ModuleProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 font-bold uppercase italic font-sans">
      ...
      <div className="grid grid-cols-2 gap-10">
        <div className="space-y-8">
          <InputGroup label="Trigger Code">
            <input type="text" value={trigger} onChange={(e) => setTrigger(e.target.value)} className="w-full bg-[#0c0c0e] border border-zinc-800 rounded-lg p-3 text-white focus:outline-none focus:border-zinc-600 not-italic" />
          </InputGroup>
          
          <div className="grid grid-cols-2 gap-4">
            <InputGroup label="Start (s)">
              <input type="number" value={start} onChange={(e) => setStart(e.target.value)} className="w-full bg-[#0c0c0e] border border-zinc-800 rounded-lg p-3 text-white focus:outline-none focus:border-zinc-600 not-italic" />
            </InputGroup>
            <InputGroup label="Ende (s)">
              <input type="number" value={end} onChange={(e) => setEnd(e.target.value)} className="w-full bg-[#0c0c0e] border border-zinc-800 rounded-lg p-3 text-white focus:outline-none focus:border-zinc-600 not-italic" />
            </InputGroup>
          </div>

          <div className="space-y-4">
            <label className="text-[10px] text-zinc-500 uppercase tracking-widest font-black italic">Audio Boost</label>
            ...
            <div className="flex justify-between text-[10px] text-zinc-600 font-mono italic"><span>0%</span><span>{vol}%</span><span>500%</span></div>
          </div>
        </div>

        <InputGroup label="Video URL (.MP4)">
          <textarea 
            value={url} 
            onChange={(e) => setUrl(e.target.value)} 
            placeholder="https://..."
            className="w-full bg-[#0c0c0e] border border-zinc-800 rounded-lg p-4 text-zinc-400 font-mono text-[10px] min-h-[180px] focus:outline-none focus:border-zinc-600 not-italic"
          />
        </InputGroup>
      </div>
      ...
      <div className="bg-[#0c0c0e] border border-zinc-800 rounded-2xl p-8 space-y-4 not-italic font-bold">
        <div className="flex gap-2">
          <div className="flex-1 text-zinc-600 truncate bg-black px-4 py-4 rounded-xl border border-white/5 font-mono text-[10px] uppercase">
            {link}
          </div>
          <button onClick={() => navigator.clipboard.writeText(link)} className="bg-white text-black px-12 rounded-xl font-black uppercase text-xs hover:bg-zinc-200 transition-all">Copy</button>
        </div>
      </div>
    </div>
  );
...
function SidebarItem({ icon, label, active, onClick }: any) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[11px] uppercase transition-all tracking-widest font-normal ${active ? "bg-[#0c0c0e] text-white font-bold border border-white/5 shadow-xl" : "text-zinc-500 hover:text-white"}`}>
...
function InputGroup({ label, children }: any) {
  return <div className="space-y-3"><label className="text-[10px] text-zinc-500 uppercase tracking-widest font-black italic">{label}</label>{children}</div>;
}
