  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#09090b] text-zinc-200 font-sans text-[12px]">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-black border-r border-white/10 z-50 transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex flex-col h-full font-sans">
          ...
          <div className="p-3 border-t border-white/5 space-y-1">
            ...
            <button className="w-full flex items-center gap-3 px-3 py-2 text-[11px] text-red-500/60 hover:text-red-500 transition-all uppercase font-bold tracking-widest mt-2">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#09090b]">
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 backdrop-blur-md sticky top-0 z-30 font-sans font-bold uppercase tracking-widest">
...
function ModuleSettings({ expiry, version, isLogged, setIsLogged, username }: SettingsProps) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.url) window.location.href = data.url; // Öffnet echte TikTok API Seite
    ...
    <div className="p-10 max-w-5xl mx-auto space-y-8 font-bold uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-bold uppercase">
        ...
        <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 flex flex-col justify-between shadow-2xl">
          ...
          {isLogged ? (
            <div className="mt-4 p-4 bg-green-500/10 border border-green-500/20 rounded text-green-500 text-[10px] flex items-center justify-center gap-2 font-black uppercase tracking-widest">
              <CheckCircle size={14} /> Logged in as @{username}
            </div>
          ) : (
            <button onClick={handleLogin} disabled={loading} className="mt-4 w-full bg-white text-black py-4 rounded-lg font-black text-[10px] flex items-center justify-center gap-2 hover:bg-zinc-200 transition-all uppercase tracking-widest shadow-xl">
              {loading ? <Loader2 className="animate-spin" /> : <Lock size={14} />} Connect TikTok
            </button>
          )}
        </div>
      </div>
...
function ModuleFanClub({ username, isLogged }: FanClubProps) {
  const stickers = [
    ...
    { name: "Raygun", icon: "🔫" }, { name: "Pack-a-Punch", icon: "🌀" }
  ];
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-10 font-bold uppercase">
      ...
      {!isLogged ? (
        <div className="bg-zinc-900/50 border border-zinc-800 p-16 rounded-2xl text-center space-y-4 shadow-2xl">
          ...
          <p className="text-zinc-500 text-sm italic">Start synchronization in settings.</p>
        </div>
      ) : (
...
function ModuleTTV({ username, baseUrl }: ModuleProps) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-10 font-bold uppercase">
      <h2 className="text-2xl font-black text-white">TTV Setup</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-bold uppercase">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex gap-2">
        ...
        <button onClick={() => {navigator.clipboard.writeText(link); setCopied(true); setTimeout(() => setCopied(false), 2000);}} className="bg-white text-black px-8 rounded font-black text-[10px] uppercase">{copied ? "Copied" : "Copy"}</button>
      </div>
    </div>
  );
}
...
function ModuleSoundAlerts() { return <div className="p-8 text-zinc-600 font-bold uppercase italic">Sound alerts coming soon...</div>; }
