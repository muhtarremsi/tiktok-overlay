...
import { 
  ...
  Users, Volume2, Globe, CheckCircle
} from "lucide-react";

// TikTok Logo Komponente
const TikTokIcon = () => (
...
interface SettingsProps { expiry: string; version: string; isLogged: boolean; setIsLogged: (v: boolean) => void; username: string; lang: string; setLang: (l: string) => void; }
...
function DashboardContent() {
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case "settings": return <ModuleSettings expiry={expiryDate} version={version} isLogged={isLogged} setIsLogged={setIsLogged} username={username} lang={lang} setLang={updateLang} />;
    ...
    <div className="flex h-screen w-full overflow-hidden bg-[#09090b] text-zinc-200 font-sans text-[12px]">
        ...
        <div className="flex flex-col h-full p-5">
          ...
          <div className="border-t border-white/5 pt-4 space-y-2">
            ...
            <div className="relative group px-3 py-2 text-zinc-500 flex items-center justify-between hover:text-white cursor-pointer uppercase font-bold tracking-widest transition-all">
              ...
              <span className="text-[10px] text-zinc-400 font-mono">{lang}</span>
              <div className="absolute bottom-full left-0 w-full bg-zinc-900 border border-zinc-800 rounded hidden group-hover:block z-50">
                {["EN", "DE", "RU"].map(l => (
                  <button key={l} onClick={() => updateLang(l)} className="w-full px-4 py-2 text-left hover:bg-white/5 text-[10px] uppercase">{l}</button>
                ))}
              </div>
            </div>
      ...
      <main className="flex-1 flex flex-col bg-[#09090b]">
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 font-bold uppercase tracking-widest text-[10px]">
          ...
          <div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${isLive ? 'bg-green-500' : 'bg-zinc-800'}`} /> {username}</div>
        </header>
    ...
    <div className="p-10 max-w-5xl mx-auto space-y-8 uppercase font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 flex flex-col justify-between">
          ...
          {isLogged ? <div className="text-green-500 text-[10px] mt-4 flex items-center gap-2"><CheckCircle size={14} /> Connected: @{username}</div> : 
          <button onClick={handleLogin} disabled={loading} className="mt-4 bg-white text-black py-4 rounded-xl text-[11px] font-black uppercase flex items-center justify-center gap-3 hover:bg-zinc-200 transition-all shadow-xl">
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-10 font-bold uppercase italic">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex gap-2 not-italic">
        ...
        <button onClick={() => navigator.clipboard.writeText(link)} className="bg-white text-black px-8 rounded font-black text-[10px]">COPY</button>
      </div>
    </div>
  );
}

function ModuleFanClub() { return null; }
