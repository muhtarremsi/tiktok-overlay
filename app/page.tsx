...
export default function Dashboard() {
  ...
  // --- FIX: ZUVERLÄSSIGER STATUS-CHECK ---
  useEffect(() => {
    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        // Setze den Status basierend auf der API-Antwort
    ...
    checkStatus();
    const interval = setInterval(checkStatus, 15000); // Check alle 15 Sek.
    ...
    <div className="flex h-screen w-full overflow-hidden bg-[#09090b] text-zinc-200 font-sans text-[12px]">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#09090b] relative font-sans">
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 backdrop-blur-md sticky top-0 z-30">
          ...
          {isLive && <span className="px-2 py-0.5 rounded bg-green-500/10 border border-green-500/20 text-[9px] text-green-500 font-bold uppercase animate-pulse">LIVE</span>}
        </header>
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-10 uppercase font-bold">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-6">
          ...
          <InputGroup label="Audio Boost"><input type="range" min="0" max="500" value={vol} onChange={(e) => setVol(e.target.value)} className="w-full accent-white h-1.5 cursor-pointer" /></InputGroup>
        </div>
        <InputGroup label="Video URL"><textarea value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} className="input-field min-h-[150px] font-mono text-[10px] normal-case" placeholder="https://..." /></InputGroup>
      </div>
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex gap-2 shadow-2xl">
        ...
        <button onClick={() => {navigator.clipboard.writeText(link); setCopied(true); setTimeout(() => setCopied(false), 2000);}} className="bg-white text-black px-8 rounded font-black text-[10px] uppercase transition-all active:scale-95">
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
    </div>
...
function ModuleFanClub({ username, isLogged }: FanClubProps) {
  const stickers = [
    ...
    { name: "Raygun", icon: "🔫" }, { name: "Juggernog", icon: "🥤" }
  ];

  return (
    <div className="p-8 max-w-5xl mx-auto space-y-10 font-bold uppercase">
      <h2 className="text-2xl font-black text-white">Fan Club Manager</h2>
      {!isLogged ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-8 space-y-6 shadow-2xl">
            <h3 className="text-[10px] text-zinc-400 uppercase tracking-widest font-black">Team Sticker</h3>
            <div className="grid grid-cols-2 gap-3">
              {stickers.map((s, i) => (
                <div key={i} className="aspect-square bg-zinc-900 rounded-xl flex items-center justify-center border border-white/5 hover:border-blue-500 transition-all cursor-pointer shadow-lg group">
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8 font-black uppercase">
      ...
      <div className="bg-zinc-900/30 border border-zinc-800 p-8 rounded-2xl space-y-6">
        <div className="max-w-xs space-y-3 font-bold uppercase">
          <label className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">Master Volume: {vol}%</label>
          <input type="range" min="0" max="500" value={vol} onChange={(e) => setVol(e.target.value)} className="w-full accent-white h-1.5 cursor-pointer" />
        </div>
        <div className="bg-black border border-zinc-800 rounded-xl p-6 space-y-4">
          ...
          <div className="flex gap-2 font-mono text-[10px] uppercase">
            <div className="flex-1 text-zinc-500 truncate bg-zinc-900 px-4 py-3 rounded border border-white/5 lowercase">{link}</div>
            <button onClick={() => {navigator.clipboard.writeText(link); setCopied(true); setTimeout(() => setCopied(false), 2000);}} className="bg-white text-black px-8 rounded font-black text-[10px] uppercase transition-all active:scale-95">
              {copied ? "Copied" : "Copy"}
            </button>
          </div>
...
function ModuleSettings({ expiry, version, isLogged, setIsLogged, username }: SettingsProps) {
  ...
  const handleLogin = async () => { if(!username) return; setLoading(true); await fetch('/api/auth/tiktok', { method: 'POST', body: JSON.stringify({ username }) }); setTimeout(() => { setIsLogged(true); setLoading(false); }, 1500); };
  ...
  return (
    <div className="p-10 max-w-5xl mx-auto space-y-8 font-bold uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-bold uppercase">
        <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 space-y-4 shadow-2xl transition-all hover:border-zinc-700">
          <h3 className="text-blue-500 text-sm flex items-center gap-2 uppercase font-black tracking-widest"><ShieldCheck size={18} /> PRO License: Active</h3>
        ...
        <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 flex flex-col justify-between shadow-2xl transition-all hover:border-zinc-700">
          ...
          {isLogged ? (
            <div className="mt-4 p-4 bg-green-500/10 border border-green-500/20 rounded text-green-500 text-[10px] flex items-center justify-center gap-2 font-black tracking-widest uppercase">
              <CheckCircle size={14} /> Angemeldet als @{username}
            </div>
          ) : (
            <button onClick={handleLogin} disabled={loading || !username} className="mt-4 w-full bg-white text-black py-4 rounded-lg font-black text-[10px] flex items-center justify-center gap-2 hover:bg-zinc-200 transition-all disabled:opacity-30 uppercase tracking-widest shadow-xl">
...
function InputGroup({ label, children }: any) {
  return <div className="flex flex-col gap-2 font-bold uppercase"><label className="text-[9px] text-zinc-500 tracking-widest font-black uppercase">{label}</label>{children}</div>;
}
