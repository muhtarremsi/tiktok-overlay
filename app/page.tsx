    ...
    <div className="flex h-screen w-full overflow-hidden bg-[#09090b] text-zinc-200 font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-black border-r border-white/10 z-50 transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-white/5">
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/50 rounded-lg p-3 space-y-2.5">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#09090b] relative">
        <header className="h-16 border-b border-white/5 flex items-center justify-between px-4 lg:px-8 bg-black/20 backdrop-blur-md sticky top-0 z-30">
          ...
          {isLive && <span className="px-2 py-0.5 rounded-full bg-green-500/10 border border-green-500/20 text-[10px] text-green-500 font-bold">LIVE</span>}
        </header>
...
function ModuleTTV({ username }: { username: string }) {
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto space-y-8">
      <h2 className="text-xl sm:text-2xl font-semibold text-white">Konfiguration</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-6">
          <InputGroup label="Trigger" desc="Chat-Code"><input type="text" value={trigger} onChange={(e) => setTrigger(e.target.value)} className="input-field" /></InputGroup>
        ...
        <InputGroup label="Video URL" desc=".mp4 Link"><textarea value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} className="input-field min-h-[100px] font-mono text-[11px]" /></InputGroup>
      </div>
...
function SidebarItem({ icon, label, active, onClick }: any) {
  return <button onClick={onClick} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm transition-all ${active ? "bg-zinc-900 text-white font-medium shadow-sm" : "text-zinc-500 hover:bg-zinc-900/50"}`}><span>{icon}</span>{label}</button>;
}
function InputGroup({ label, desc, children }: any) {
  return <div className="flex flex-col gap-2"><label className="text-xs font-bold text-zinc-400 uppercase tracking-tighter">{label}</label>{children}<p className="text-[10px] text-zinc-600 italic">{desc}</p></div>;
}
function ModuleSettings({ expiry, version }: any) {
  return <div className="p-8"><div className="bg-zinc-900/50 p-6 rounded-lg border border-zinc-800 max-w-lg"><h3 className="text-white font-medium mb-4 flex items-center gap-2"><Zap size={16} className="text-yellow-500" /> PRO Lizenz</h3><p className="text-zinc-400 text-sm">Gültig bis: {expiry}</p><p className="text-zinc-600 text-[10px] mt-2 font-mono">Build: {version}</p></div></div>;
}
function ModulePlaceholder({ title }: any) {
  return <div className="p-8 text-zinc-600 uppercase text-[10px] tracking-widest">{title} wird geladen...</div>;
}
