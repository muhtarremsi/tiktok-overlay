...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#09090b] text-zinc-200 font-sans text-[12px]">
        ...
        <div className="flex flex-col h-full font-sans font-bold uppercase tracking-widest">
          <div className="p-5 border-b border-white/5">
            <div className="flex items-center justify-between mb-5 text-white">
              <h1 className="text-base flex items-center gap-2 font-black tracking-tighter">
            ...
            <div className="relative mb-5">
              ...
              <input type="text" placeholder="TikTok User" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 text-zinc-200 text-xs rounded py-2 pl-6 pr-8 focus:outline-none focus:border-zinc-500 font-bold uppercase" />
            ...
            <div className="bg-zinc-900/40 border border-zinc-800/60 rounded-lg p-3 space-y-2.5 text-[9px] font-black">
              ...
              <div className="flex items-center justify-between border-t border-white/5 pt-2"><span>ABLAUF</span><span className="text-zinc-300 font-bold">{expiryDate}</span></div>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#09090b] relative font-sans font-bold uppercase">
        <header className="h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/20 backdrop-blur-md sticky top-0 z-30 font-sans">
          <div className="flex items-center gap-4 text-[10px]">
            ...
            <div className="text-zinc-500 uppercase tracking-widest font-sans leading-none">App / <span className="text-white font-sans">{activeView}</span></div>
          </div>
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-10 font-bold uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="space-y-6">
          <InputGroup label="Trigger Code"><input type="text" value={trigger} onChange={(e) => setTrigger(e.target.value)} className="input-field text-blue-400 font-black uppercase" /></InputGroup>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex gap-2">
        <div className="flex-1 bg-black border border-zinc-800 rounded px-4 py-3 text-zinc-400 font-mono text-[10px] truncate uppercase">{link}</div>
        <button onClick={() => {navigator.clipboard.writeText(link); setCopied(true); setTimeout(() => setCopied(false), 2000);}} className="bg-white text-black px-6 rounded font-black text-[10px] uppercase">
          {copied ? "Kopiert" : "Copy"}
        </button>
      </div>
    </div>
...
function ModuleFanClub({ username, isLogged, setIsLogged }: FanClubProps) {
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-10 font-bold uppercase">
      <div className="flex items-center justify-between uppercase font-black"><h2 className="text-2xl text-white">Fan Club Manager</h2>{isLogged && <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/20 px-3 py-1 rounded text-green-500 text-[10px]"><CheckCircle size={14} /> Synchronisiert</div>}</div>
      {!isLogged ? (
        <div className="bg-zinc-900/50 border border-zinc-800 p-12 rounded-2xl text-center space-y-4">
          ...
          <p className="text-zinc-500 text-sm normal-case font-bold">Bitte melde dich in den <span className="text-white font-bold">Settings</span> bei TikTok an.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 font-bold uppercase">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <h3 className="text-[10px] text-zinc-500 uppercase mb-4 tracking-widest font-black font-sans">Team Sticker</h3>
...
function ModuleSoundAlerts({ username, baseUrl }: ModuleProps) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8 font-black uppercase">
      <h2 className="text-2xl text-white tracking-tight">Sound Alerts</h2>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-3">
          {Object.keys(sounds).map((s) => (
            <button key={s} onClick={() => setSelected(s)} className={`w-full p-3 rounded-lg border text-left flex items-center justify-between text-[11px] font-black uppercase transition-all ${selected === s ? "bg-blue-500/10 border-blue-500 text-white" : "bg-zinc-900 border-zinc-800 text-zinc-500"}`}>
              <span>{s}</span><Music size={14} />
            </button>
          ))}
        </div>
        <div className="lg:col-span-2 space-y-6 bg-zinc-900/30 border border-zinc-800 p-6 rounded-2xl uppercase">
          <InputGroup label="Trigger Wort"><input type="text" value={sounds[selected].trigger} onChange={(e) => setSounds({...sounds, [selected]: {...sounds[selected], trigger: e.target.value}})} className="input-field text-blue-400 font-black uppercase" /></InputGroup>
          <div className="bg-black border border-zinc-800 rounded-xl p-4 flex gap-2 font-bold font-mono text-[9px] uppercase">
            <div className="flex-1 text-zinc-500 truncate">{link}</div>
            <button onClick={() => {navigator.clipboard.writeText(link); setCopied(true); setTimeout(() => setCopied(false), 2000);}} className="bg-white text-black px-4 rounded font-black text-[9px] uppercase tracking-tighter">{copied ? "Copied" : "Copy"}</button>
          </div>
        </div>
      </div>
    </div>
...
function ModuleSettings({ expiry, version, isLogged, setIsLogged, username }: SettingsProps) {
  ...
  const handleLogin = () => { if(!username) return; setLoading(true); setTimeout(() => { setIsLogged(true); setLoading(false); }, 1500); };

  return (
    <div className="p-10 max-w-5xl mx-auto space-y-8 font-black uppercase">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-black">
        <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 space-y-4 shadow-2xl">
          <h3 className="text-blue-500 text-sm flex items-center gap-2 uppercase"><ShieldCheck size={18} /> PRO License: Active</h3>
          <div className="space-y-1 uppercase"><p className="text-zinc-200 text-sm font-bold uppercase">Ablauf: {expiry}</p><p className="text-zinc-600 text-[10px] font-mono uppercase font-black">Build Version: {version}</p></div>
        </div>
        <div className="bg-zinc-900/50 p-8 rounded-xl border border-zinc-800 flex flex-col justify-between">
          <div className="space-y-2 uppercase">
            ...
            <p className="text-zinc-500 text-[10px] font-black uppercase tracking-tighter">Verbinde deinen Account für Sticker-Synchronisation.</p>
          </div>
          {isLogged ? (
            <div className="mt-6 p-3 bg-green-500/10 border border-green-500/20 rounded flex items-center justify-center gap-2 text-green-500 text-[10px] font-black uppercase">
              <CheckCircle size={14} /> Angemeldet als @{username}
            </div>
          ) : (
            <button onClick={handleLogin} disabled={loading || !username} className="mt-6 w-full bg-white text-black py-3 rounded-lg font-black text-[10px] flex items-center justify-center gap-2 hover:bg-zinc-200 transition-all disabled:opacity-30 uppercase">
...
function SidebarItem({ icon, label, active, onClick }: any) {
  return <button onClick={onClick} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-[10px] transition-all font-black uppercase tracking-widest ${active ? "bg-zinc-900 text-white" : "text-zinc-500 hover:bg-zinc-900/50"}`}><span>{icon}</span>{label}</button>;
}
function InputGroup({ label, children }: any) {
  return <div className="flex flex-col gap-2 font-black uppercase tracking-widest"><label className="text-[9px] text-zinc-400 uppercase tracking-widest">{label}</label>{children}</div>;
}
